...
describe('testing admin related APIs', async () => {
  ...
  describe('responsive name search', () => {
    test('pending user - case insensitive', async () => {
      expect.assertions(2)
      const response = await app.get('/admin/search/cRIs')
      expect(response.statusCode).toBe(200)
      expect(response.body).toEqual({'pendingMatched': [], 'activeMatched': [{'profile': {'name': 'Mr. Cristian Bartell'}, 'status': 'Active', 'roles': ['Tutor'], '_id': '5b9255700333773af993ae9c'}], 'suspendedMatched': [], 'deletedMatched': []})
    })
    ...
    test('multiple match', async () => {
      ...
      expect(response.statusCode).toBe(200)
      expect(response.body).toEqual({'pendingMatched': [], 'activeMatched': [{'profile': {'name': 'Wuying  Kong'}, 'status': 'Active', 'roles': ['Tutor', 'SuperAdmin', 'Admin'], '_id': '5b912ba72b9ec042a58f88a4'}], 'suspendedMatched': [{'profile': {'name': 'Marie G Kruger'}, 'status': 'Suspended', 'roles': ['Tutor', 'SuperAdmin', 'Admin'], '_id': '5b9255260333773af993ae9b'}], 'deletedMatched': []})
    })
    ...
    test('deleted match', async () => {
      ...
      expect(response.statusCode).toBe(200)
      expect(response.body).toEqual({'pendingMatched': [], 'activeMatched': [], 'suspendedMatched': [], 'deletedMatched': [{'profile': {'name': 'Admin1'}, 'status': 'Deleted', 'roles': ['Tutor', 'SuperAdmin', 'Admin'], '_id': '5a1111574f52053930820a42'}]})
    })
  })
  ...
  describe('changing role and status of user', () => {
    ...
    test('correct userId shows success and changes classes accordingly', async () => {
      ...
      const userData = await app.get('/users/5b912ba72b9ec042a58f88a4').set('x-access-token', userToken)
      expect(userData.statusCode).toBe(200)
      expect(userData.body.user.status).toBe('Active')
      expect(userData.body.user.roles).toEqual(['SuperAdmin', 'Admin'])
      const classData = await app.get('/class/5b97b8f2adfb2e018c64d372').set('x-access-token', userToken)
      expect(classData.statusCode).toBe(200)
      expect(classData.body.class.users).toEqual([
        {
